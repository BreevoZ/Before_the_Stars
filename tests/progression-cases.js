...
export function registerProgressionTests(test, assert, near) {
  ...
  test('Debug: commands are gated, use normal one-time settlement, and retain upgrades across rebuild', () => {
    ...
    assert(runDebugCommand(s, 'finale')); rebuildCivilization(s, s.run.runId); assert(runDebugCommand(s, 'finale'));
    ...
    assert(runDebugCommand(s, 'defeat') && s.run.phase === 'defeat' && s.permanent.completedCycles === 3);
  ...
  test('M1: upgrade costs, caps and between-run restriction; modifiers never touch shared data or enemies', () => {
    ...
    finish(s);
    s.permanent.totalLegacy = s.permanent.completedCycles = s.permanent.legacy = 125; s.permanent.automation.unlocked = true;
    assert(purchaseTalent(s, 'spark'));
    for (const key of ['production', 'warfare']) {
      for (const cost of [2, 4, 8, 16, 32]) {
  ...
  test('M1: actual casualty payouts round base reward then multiplier, without multiplying kill gold', () => {
    for (const victimTeam of ['player', 'enemy']) {
      const s = createProgression(); finish(s); purchaseTalent(s, 'spark'); rebuildCivilization(s, s.run.runId); finish(s); rebuildCivilization(s, s.run.runId); finish(s); assert(purchaseUpgrade(s, 'warfare')); rebuildCivilization(s, s.run.runId);
  ...
  test('M1: settlement, victory and rebuilt saves round-trip without repeating awards, refunds or multipliers', () => {
    ...
    rebuildCivilization(s, s.run.runId); finish(s); assert(purchaseUpgrade(s, 'production')); setAutomation(s, true, 'ranged');
    ...
    assert(s.permanent.completedCycles === 3 && s.permanent.legacy === 0 && statMultiplier(s.game, 'income') === 1.5);
  ...
  test('Super soldier training and both attack poses survive full save round trips', () => {
    const seed = createProgression(); finish(seed); seed.permanent.totalLegacy = seed.permanent.completedCycles = seed.permanent.legacy = 1000; seed.permanent.automation.unlocked = true; purchaseTalent(seed,'spark'); buyUnitPath(seed); rebuildCivilization(seed, seed.run.runId); seed.game.ai.enabled = false; ageTo(seed.game, 5);
  ...
  test.browser('Debug browser: fast fixed-step simulation pauses; ending, upgrading, rebuilding and reloading work in isolation', async () => {
    ...
    command('finale'); el('rebuild-civilization').click(); command('finale'); el('result-talents').click(); el('node-production').click();
    ...
    command('defeat'); el('play-again').click(); assert(el('cycles').textContent === '3' && el('legacy').textContent === '0');
