...
export const TALENTS = Object.freeze({
  spark: { name: '文明火种', branch: 'root', costs: [1], requires: {},
    effects: ['游戏速度 1×', '解锁 1× / 2× 速度切换 · 自动招募在累计通关 2 次后免费解锁'] },
  logistics: { name: '后勤调度', branch: 'automation', costs: [1], requires: { spark: 1 },
    effects: ['使用默认预算与队列', '自定义预留金币、队列上限与购买优先级'] },
  formation: { name: '编队协议', branch: 'automation', costs: [1], requires: { spark: 1 },
  ...
  elite: { name: '精锐征召', branch: 'automation', costs: [3], requires: { superSoldierPlan: 1 },
  ...
  timeAcceleration: { name: '时间加速', branch: 'growth', layer: 4, costs: [8], requires: { spark: 1 }, requiresLayer: 3,
