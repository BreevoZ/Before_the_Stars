...
export function registerTalentTests(test, assert, near) {
  ...
  test('Autobuyer full civilization: purchased talents, normal paid armies and defense reach the finale without manual combat commands', () => {
    ...
    assert(s.run.phase === 'destruction' && s.run.earnedLegacy === getLegacyReward(s.run.talents) && s.permanent.completedCycles === 200001);
  });
  ...
  test.browser('Talent tree browser: real parent branches carry required ranks and selecting a node shows only its detail', async () => {
    ...
    el('node-legacyMachine').click(); assert(el('talent-legacyMachine').textContent.includes('遗产保存 1 级'));
  ...
  test('Single root: all three routes require the one-Legacy Autobuyer root and every node has one path back to it', () => {
    ...
    // The producer needs both its reward prerequisite and a Renaissance trait.
    assert(!purchaseTalent(s, 'legacyMachine') && purchaseTalent(s, 'conservation'));
    assert(!purchaseTalent(s, 'legacyMachine'));
    for (const key of ['openingStone', 'shieldWall', 'parry']) assert(purchaseTalent(s, key));
    assert(purchaseTalent(s, 'legacyMachine') && purchaseTalent(s, 'legacyCapacity'));
  });
  test('Save v4: v3 growth or legacy owners receive the required root without losing currency, replaying rewards or changing battle bonuses', () => {
    for (const phase of ['fresh', 'no-upgrades', 'battle', 'victory', 'destruction', 'defeat']) {
      ...
      assert(restored.run.earnedLegacy === old.run.earnedLegacy && JSON.stringify(canonical({ ...restored.game, bonuses: restored.game.bonuses.filter(b => !['legacy','legacyMachine','legacyMachineShare','legacyFillSeconds','legacyProduction','legacyProductionInterval'].includes(b.target.stat)) })) === JSON.stringify(canonical({ ...s.game, bonuses: s.game.bonuses.filter(b => !['legacy','legacyMachine','legacyMachineShare','legacyFillSeconds','legacyProduction','legacyProductionInterval'].includes(b.target.stat)) })));
