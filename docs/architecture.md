...
- `orbital-config.js`：8 个点位、天赋树、前置、费用、干预和战争数值。Legacy 是唯一轨道货币，月球前哨保留战争倍率，并以模拟时钟自动生产 Legacy；生存空间与月面工场每级翻倍，不引入新资源或第二层 prestige。
- `orbital-save.js`：v16–v18 形状、前置、支付账本、文明/战争互相引用及结算一致性。`save-battle.js` 是地表与轨道共用的完整战斗校验；存档省略可推导加成、基地上限，保留部队、订单、弹药快照、双方 AI 与随机源。
- `orbital-view-model.js` / `orbital-colony-ui.js`：纯视图投影、一次绑定、全屏 SVG 天赋树与原有 Canvas 战斗监控。`orbital-render.js` 绘制自转地球、核毁灭、逐段居住环、月面设施与轨道星图。`celestial-clock.js` 统一 120 秒太阳日，按点位经度给地面渲染传入 `skyTime`，不改变真实战斗时钟。
...
v16 → v17 是保留战争的增量迁移：增加 `lunarIndustry` 等级、`lunarProduced` 和 `lunarFraction`；不改已有回收等级或支付记录，不追溯补发月面收入。`updateOrbital` 在暂停/隐藏/有效时间检查之后，按产能积累小数余量、整数入统一 Legacy 账本，核冬天照常生产。新等级通过正常购买与立即保存路径，渲染无经济副作用。

首次毁灭演出以已结算存档为前提。空布局回退到全屏战场；绘制失败时主动恢复 `inert`、移除遮罩与相关样式，保留星图和重建入口，不能让 Canvas 错误截断主循环。浏览器回归覆盖首次立即结算、零尺寸布局、注入绘制异常、跳过及后续重建推进。


v17 → v18 新增文明输入 `doctrine: 0..5`、`superSoldiers: 0..2`，不持久化派生的特性加成。`warBonuses` 将每一档学说映射到该时代三个 `trait_*` 布尔属性，同时显式开启该方 `traitAccess`；地表敌方默认关闭，经典行为不变。超级士兵复用 `getSuperSoldierBonuses`，仅轨道获授权方允许敌方精锐招募；指挥官通过正常 `recruit` 训练一名，缺钱时维持少量护卫并储蓄。禁止在干预时直接生成部队。共享钩子和力场保护按所属阵营执行，刷新加成保留生命比例和在途弹药快照。

v18 原始存档先按冻结的 v17 配置校验，再纯数据迁移：新增等级／授权为零、环上限七级，所有历史支付保持不动。`orbital-save-history.js` 只保存历史费用和前置，不复制画面配置。已有旧价允许保留，后续等级按现价购买。`tests/orbital-doctrine-cases.js` 覆盖双方的十五项特性实战、逐档隔离、付费训练、狙击引导往返、核毁灭重生、迁移账本以及窄屏交互。

星图分区与节点形状由天赋元数据描述；连线只在各路线内部延伸。背景调用 `drawOrbitalScene(..., ORBITAL_SECONDS)` 并按画布尺寸缓存，与模拟和动画时钟无关。星环以七个连续圆弧组成一周，前后两遍绘制以遮挡地球，建筑展开仅是视觉进度。月面设施使用经纬度投影、近远排序与边缘缩短；自转跟随模拟时间，减少动态效果下冻结。所有渲染函数不修改经济或存档。
