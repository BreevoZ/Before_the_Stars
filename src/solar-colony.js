...
const BOTTOM = 2500, COL = 125, ROW = 150, TIER_ROWS = Object.freeze([2, 1, 2, 2, 2]);
...
const moon = (id, region, [col, row], name, icon, parent, cost, description, extra = {}) => place(region, col, row, name, icon, { satellite: id, kind: 'keystone', costs: cost ? [cost] : [], requires: { [parent]: 1 }, description, ...extra });
...
export const SOLAR_TALENTS = Object.freeze({
  ...
  deimos: moon('deimos', 'mars', [1, 1], '火卫二', 'berth', 'harbor', 512 * M, '转运泊位：转运方舟在火卫二减速入轨：文明转运的航程缩短 20%。'),
  ...
  io: moon('io', 'jupiter', [0, 1], '木卫一', 'volcano', 'jupiter', 8 * G, '火山热电：木卫一的火山为采集站供热：木星气态采集站产出翻倍。'),
  europa: moon('europa', 'jupiter', [1, 1], '木卫二', 'ocean', 'jupiter', 16 * G, '冰下海洋：冰壳之下的海洋让升格文明找到新的研究方向：升格文明的产出 ×1.5。'),
  ganymede: moon('ganymede', 'jupiter', [2, 1], '木卫三', 'magnet', 'jupiter', 16 * G, '磁层船坞：在木卫三的磁层里铸造方舟：多一艘停泊在火星港的方舟。'),
  callisto: moon('callisto', 'jupiter', [3, 1], '木卫四', 'depot', 'jupiter', 32 * G, '外缘补给站：辐射带之外的补给站：所有方舟与转运方舟的航程再缩短 20%。'),
  ...
  enceladus: moon('enceladus', 'saturn', [0, 1], '土卫二', 'plume', 'saturn', 32 * G, '冰羽采集：收集土卫二喷出的冰羽：土星冰环采集站产出翻倍。'),
  titan: moon('titan', 'saturn', [1, 1], '土卫六', 'lake', 'saturn', 0, '甲烷湖前哨：厚雾之下的甲烷湖，是火星之后的下一个殖民世界。（后续开放）', { planned: true }),
  uranus: world('uranus', '冰巨星采集站', 'icegiant', 'deepDrive', { facility: 'uranus' }),
  titania: moon('titania', 'uranus', [0, 1], '天卫三', 'crystal', 'uranus', 128 * G, '冰岩采集：天卫三的冰岩里藏着更多氘：冰巨星采集站产出翻倍。'),
