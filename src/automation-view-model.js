...
export const AUTOMATION_FIELDS = Object.freeze({
  ...
  'auto-ability': ['ability', 'checked'], 'auto-campaign': ['campaign', 'checked'],
...
export function buildAutomationViewModel(session, { mobile = false } = {}) {
  ...
  put('talent-guide', !p.talents.spark ? '从根节点点亮第一颗星。花 1 Legacy 点亮文明火种，解锁 2× 速度。自动招募在累计通关 2 次后免费获得。' : mobile ? '上下探索时代，左右浏览侧枝 · 点击节点查看与购买' : '每个时代点亮任意一个兵种天赋即可进入下一时代 · 发光的节点可购买 · 双击购买');
  ...
  for (const [key, level] of [['fireControl', 1], ['campaign', 1], ['logistics', 1], ['formation', 1], ['elite', 1], ['evolution', 1], ['defense', 1], ['defense', 2]]) {
  ...
  for (const [id, required, hint] of [['ability', p.talents.fireControl, '需战术火控'], ['campaign', p.talents.campaign, '需连续征战'], ['evolve', p.talents.evolution, '需技术托管'], ['defense', p.talents.defense, '需防御工程'], ['replace', p.talents.defense >= 2, '需防御工程 2 级'], ['elite', p.talents.elite, '需精锐征召']]) {
  ...
  put('automation-hint', '每 0.25 秒模拟时间检查一次。通过天赋解锁功能，再自行启用；暂停、隐藏页面和弹窗打开时停止。连续征战仅继续普通战役，不自动重建文明。');
