...
export function buildTalentViewModel(session) {
  ...
  for (const [name, config] of Object.entries(TALENT_TREE)) {
    ...
    const price = config.costs[level] === undefined ? '' : Q.format(config.costs[level], 6);
    const reason = session.run.phase === 'orbital' && state === 'during-run' ? '已抵达轨道 · 地表档案留存' : state === 'ready' && name === 'bypasser' ? '购买后立即启航 · 演出可跳过，进度立即保存' : state === 'ready' && ['spark', 'timeAcceleration'].includes(name) ? '立即解锁速度档位' : reasons[state];
    put(`level-${name}`, '●'.repeat(level) + '○'.repeat(config.costs.length - level));
    put(`cost-${name}`, state === 'planned' ? '待开放' : state === 'max' ? '完整' : `${price} ✧`);
    ...
    put(`buy-${name}`, state === 'planned' ? '尚未开放' : state === 'max' ? '已满级' : `${price} Legacy · ${name === 'bypasser' ? '启航' : level ? '升级' : '点亮'}`);
